...
import LikehoodSlider from './LikehoodSlider';
...
const LikehoodChart = () => {
    ...
    const fetchData = async () => {
      try {
        const [yearResponse, intensityResponse] = await Promise.all([
          axios.get('http://localhost:3000/get-topic'),
          axios.get('http://localhost:3000/get-intensity')
        ]);

        setTopic(yearResponse.data);
  ...
  const[testing,setTesting]=useState(topic); 
  ...
  }, [chartType, topic, intensity,testing]);
