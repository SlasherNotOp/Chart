...
const RegionLikelihood = () => {
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/region-likelihood');
        const labels = response.data.map(item => item._id); // Region
        const data = response.data.map(item => item.averageLikelihood); // Average Likelihood
...
export default RegionLikelihood;
